        ...
        for (const node of ast) {
            if (node.kind === 'class') {
                ...
                const classInfo: ClassInfo = {
                    ...
                    extends: node.extends?.name || node.extends,
                    implements: (node.implements || []).map((i: any) => i.name || i),
                    properties: (node.body || [])
                        .filter((item: any) => item.kind === 'property')
                        .map((prop: any) => ({
                            name: prop.name || '',
                    ...
                    methods: (node.body || [])
                        .filter((item: any) => item.kind === 'method')
                        .map((method: any) => ({
                            name: method.name || '',
                            visibility: method.visibility || 'public',
                            isStatic: method.isStatic || false,
                            isAbstract: method.isAbstract || false,
                            parameters: (method.arguments || []).map((arg: any) => ({
                                name: arg.name || '',
                                type: arg.type,
    ...
    private detectCrossFilePOPChains(fileInfos: PhpFileInfo[]): POPChain[] {
        ...
        for (const fileInfo of fileInfos) {
            try {
                ...
                const chainsInFile = detector.findPOPChains(fileInfo.content);

                for (const chainResult of chainsInFile) {
                    // 检查链是否跨越多个文件
                    ...
                    if (chainFiles.size > 1) {
                        // 转换为 POPChain 类型
                        const popChain: POPChain = {
                            entryPoint: `${chainResult.entryClass}::${chainResult.entryMethod}`,
                            steps: chainResult.steps.map(step => ({
                                className: step.className,
                                methodName: step.methodName,
                                operation: (step as any).operation || step.methodName || '',
                                location: new vscode.Location(
                                    vscode.Uri.file(fileInfo.path),
                                    new vscode.Position(0, 0)
                                )
                            })),
                            sink: chainResult.finalSink,
                            exploitability: chainResult.riskLevel === 'critical' ? 1.0 : 0.5,
                            description: chainResult.description
                        };
                        chains.push(popChain);
                    }
                }
            } catch (error) {
